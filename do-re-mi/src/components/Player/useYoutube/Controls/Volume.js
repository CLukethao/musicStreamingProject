...
const Volume = ({ mute, muted, volume, changeVol }) => {
    ...
    return (
        <div className='vol-container' onMouseOver={onHover} onMouseOut={onHover}>

            <div className={'vol-range-container ' + (show ? 'vol-show' : 'vol-hide')}>
                <input type='range' value={volume} className='vol-range' step='1' min='0' max='100' onChange={event => changeVol(event)}/>
            </div>

            <button className='btn player-btn' onClick={(event) => {mute(); event.currentTarget.blur()}} >
                {icon}
            </button>
        </div>
    )
